...
class RobotNode(Node):
    def __init__(self):
        super().__init__('robot1_core_node')

        # 1. 텔레메트리 퍼블리셔
        self.telemetry_pub = self.create_publisher(String, '/robot1/telemetry', 10)
        ...
        self.angle = 0.0
        self.battery = 98.0
        self.get_logger().info("-> [Robot1] 순수 ROS 2 노드 기동 완료")
    ...
    def on_goal_received(self, msg: String):
        try:
            ...
            self.get_logger().info(
                f"-> [Robot1] 주행 목표 수신 성공! Target X: {payload.get('x')}, Target Y: {payload.get('y')}"
            )
